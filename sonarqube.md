...
- [Sonar Scanner CLI](#sonar-scanner-cli)
  ...
  - [Create `sonar-project.properties` from template](#REDACTED)
  - [Back up the config first](#back-up-the-config-first)
  - [Replace the `sonar.host.url` with the docker address](#REDACTED)
...
docker run -ti --rm --link sonar-postgres postgres sh \
      -c 'exec psql -h "$REDACTED" -p "$REDACTED" -U sonar'
```

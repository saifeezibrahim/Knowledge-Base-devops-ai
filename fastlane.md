...
- [Match](#match)
  ...
  - [Authenticating CI/CD to Matchfiles repo](#REDACTED)
    - [HTTPS Authorization Token](#https-authorization-token)
    - [SSH Authorization via Deploy Key](#REDACTED)
  - [Populate Matchfiles Repo](#populate-matchfiles-repo)
    - [Generate Public Certificate from .p12](#REDACTED)
    - [Upload SSL cert, p12 and mobileprovision profile to Git branch](#REDACTED)
...
```shell
export MATCH_PASSWORD="REDACTED"  # Anonymized token example by anonymize.py from DevOps-Python-tools
```
...
sed -n '/REDACTED_PRIVATE_KEY/p' > "$NAME.cer"
```
...
[:octocat: firebase/REDACTED](https://github.com/firebase/REDACTED)

...
- [DevOps Bash tools for AWS, EKS, EC2 etc](#REDACTED)
- [Multi-Session Console](#multi-session-console)
- [AWS CLI](#aws-cli)
  - [Install AWS CLI](#install-aws-cli)
  - [Configure AWS CLI](#configure-aws-cli)
  - [Recommended: Automated Environment Switching](#REDACTED)
  - [Recommended: Easy Interactive Profile Switching](#REDACTED)
...
- [EC2](#ec2)
  ...
  - [Clone an EC2 instance for testing](#REDACTED)
  - [Add an EC2 EBS volume](#add-an-ec2-ebs-volume)
    - [Create EC2 EBS volume](#create-ec2-ebs-volume)
    - [Attach the new volume to the EC2 instance](#REDACTED)
    - [Partition and Format the new disk](#REDACTED)
    - [Mount the new volume by unchanging UUID for maximum stability](#REDACTED)
  - [Resize an EC2 EBS volume](#resize-an-ec2-ebs-volume)
  - [Create a snapshot of the volume using its ID](#REDACTED)
    - [Increase the size of the EBS volume](#REDACTED)
    - [Inside the EC2 VM - grow the partition and extend the filesystem](#REDACTED)
  - [Remove an EC2 EBS volume from a live running instance](#REDACTED)
- [IAM](#iam)
  - [IAM SSO Role References](#iam-sso-role-references)
- [RDS - Relational Database Service](#REDACTED)
...
- [ACM - AWS Certificate Manager](#acm---aws-certificate-manager)
  - [Import Externally Generated Certificate](#REDACTED)
- [Why move away from CloudWatch Logs and Metrics](#REDACTED)
- [Troubleshooting](#troubleshooting)
  - [EC2 VM becomes unresponsive and cannot SSH under high loads](#REDACTED)
  - [RDS Write Stops Working due to Status `Storage Full`](#REDACTED)
  - [ElastiCache Serverless fails to load Snapshot from S3](#REDACTED)
  - [EKS Spot - App fails to connect to DB due to race condition with Vault pod not being up yet](#REDACTED)
...
```shell
REDACTED.sh "$instance_id" "$ami_name"
```
...
```shell
UUID=REDACTED /tmp xfs defaults,nofail 0 2
```
...
```shell
REDACTED.sh "$volume_id" "before root partition expansion"
```
...
```text
{
    ...
    "Arn": "arn:aws:sts::123456789012:assumed-role/REDACTED/Hari@domain.com"
}
```
...
```text
arn:aws:sts::123456789012:assumed-role/REDACTED/Hari@domain.com
```

or even

```text
arn:aws:sts::123456789012:assumed-role/REDACTED
```
...
```text
arn:aws:iam::123456789012:role/REDACTED
```

You can also verify the role using this command:

```shell
aws iam get-role --role-name arn:aws:iam::123456789012:role/REDACTED
```
...
```text
arn:aws:iam::123456789012:role/aws-reserved/sso.amazonaws.com/eu-west-1/REDACTED
```
...
First ensure it's in pem format by [converting using OpenSSL](ssl.md#REDACTED) and
...
```shell
UUID=REDACTED       /       xfs     defaults,nofail        0       0
UUID=REDACTED       /tmp    xfs     defaults,nofail        0       2
UUID=REDACTED       /boot   xfs     defaults,nofail        0       0
...
![Typical Network Architecture](images/REDACTED.gif)
...
![Genie - I'll Give you $1 Billion if you can spend 100M a Month](images/REDACTED.jpeg)
